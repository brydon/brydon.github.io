...
import {createBookNote} from './cabin-book-note.mjs';
import {createCabinRug} from './cabin-rug.mjs';
import {createFireplaceGrate} from './cabin-fire-grate.mjs';
import {createFireplaceTools} from './cabin-fire-tools.mjs';
import {createHearthLogBox} from './cabin-hearth-wood.mjs';
...
export function furnishCabin(cabin){
  ...
  for(let row=0;row<4;row++)for(let i=0;i<readingRows[row].length;i++){
    ...
    if(row===0&&i===1)feynmanBook=book;
  }
  const bookNote=createBookNote(cabin,feynmanBook);
  ...
  box(hearth,.78,.014,.43,0,.077,.045,'#252b26');
  hearth.add(createFireplaceGrate());
  hearth.add(createFireplaceTools());
  for(let i=0;i<3;i++){
    const log=cylinder(hearth,.085,.1,.65,(i-1)*.045,.275+i*.035,.04,'#604029',7);log.rotation.z=Math.PI/2;log.rotation.y=(i-1)*.36;
    const ember=mesh(hearth,new THREE.IcosahedronGeometry(.07,0),'#e97223',(i-1)*.21,.092,.17,true);ember.scale.y=.3;
  }
  ...
  const flames=[];
  for(let i=0;i<7;i++){
    const flame=new THREE.Group();flame.position.set((i%4-1.5)*.145,.32,Math.floor(i/4)*.17-.04);hearth.add(flame);
    const h=.36+(i%3)*.10;
  ...
  // Spare firewood stays contained and clear of the front wall.
  cabin.add(createHearthLogBox());
  ...
  return {
    feynmanBookArea(){return bookNote.bookArea();},
    fallenNoteArea(){return bookNote.paperArea();},
    canReadFallenNote(){return bookNote.canRead();},
    releaseBookNote(reducedMotion){return bookNote.release(reducedMotion);},
    animateBookNote(seconds,reducedMotion,burning){bookNote.animate(seconds,reducedMotion,burning);},
